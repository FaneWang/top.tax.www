<%@ page contentType="text/html;charset=UTF-8" language="java" %>
<html>
<head>
    <%@include file="/common/header.jsp"%>
    <title>投诉受理管理</title>
    <script type="text/javascript">
        var list_url = "${basePath}nsfw/complain_listUI.action";
    </script>
    <script type="text/javascript" src="${basePath}js/jquery-ui/jquery-ui.js"></script>
    <link rel="stylesheet" href="${basePath}css/jquery-ui-css/jquery-ui.css">
</head>
<body class="rightBody">
<form name="form1" action="" method="post">
    <div class="p_d_1">
        <div class="p_d_1_1">
            <div class="content_info">
                <div class="c_crumbs"><div><b></b><strong>投诉受理管理</strong></div> </div>
                <div class="search_art">
                    <li>
                       	投诉标题：<s:textfield name="complain.compTitle" cssClass="s_text"  cssStyle="width:160px;"/>
                    </li>
                    <li>
                       	投诉时间：<s:textfield id="startTime" name="startTime" cssClass="s_text"  cssStyle="width:160px;" readonly="true"/>
                              -
                             <s:textfield id="endTime" name="endTime" cssClass="s_text"  cssStyle="width:160px;" readonly="true"/>
                    </li>
                    <li>
                       	状态：<s:select name="complain.compState" list="#complainState" headerKey="" headerValue="全部"/>
                    </li>
                    <li><input type="button" class="s_button" value="搜 索" onclick="doSearch()"/></li>
                    <li style="float:right;">
                    	<input type="button" value="统计" class="s_button" onclick="doAnnualStatistic()"/>&nbsp;
                    </li>

                </div>

                <div class="t_list" style="margin:0px; border:0px none;">
                    <table width="100%" border="0">
                        <tr class="t_tit">
                            <td align="center">投诉标题</td>
                            <td width="120" align="center">被投诉部门</td>
                            <td width="120" align="center">被投诉人</td>
                            <td width="140" align="center">投诉时间</td>
                            <td width="100" align="center">受理状态</td>
                            <td width="100" align="center">操作</td>
                        </tr>
                       <s:iterator value="pageResult.list" status="st">
                            <tr <s:if test="#st.odd">bgcolor="#ffe5e5"</s:if><s:else>bgcolor="#eeffe5"</s:else> >
                                <td align="center"><s:property value="compTitle"/></td>
                                <td align="center"><s:property value="toCompDept"/></td>
                                <td align="center"><s:property value="toCompName"/></td>
                                <td align="center"><s:date name="comTime" format="yyyy-MM-dd HH:mm"/></td>
                                <td align="center"><s:property value="#complainState[compState]"/></td>
                                <td align="center">
                                    <s:if test="compState != 2">
                                        <a href="javascript:doDeal('<s:property value='compId'/>')">受理</a>
                                    </s:if>
                                    <s:elseif test="compState == 2">
                                        <a href="javascript:doSee('<s:property value='compId'/>')">查看</a>
                                    </s:elseif>
                                </td>
                            </tr>
                        </s:iterator>
                    </table>
                </div>
            </div>

            <jsp:include page="/common/pagination.jsp"></jsp:include>

        </div>
    </div>
</form>
<script type="text/javascript">
    $("#startTime").datepicker({
        changeMonth: true,
        changeYear: true,
        dateFormat:"yy-mm-dd"
    });

    $("#endTime").datepicker({
        changeMonth: true,
        changeYear: true,
        dateFormat:"yy-mm-dd"
    });

    function doSearch(){
        $("#pageNo").val(0);
        var form1 = $("form[name='form1']")[0];
        form1.action = list_url;
        form1.submit();
    }

    function doDeal(id){
        var form1 = $("form[name='form1']")[0];
        form1.action = "${basePath}nsfw/complain_dealUI.action?complain.compId=" + id;
        form1.submit();
    }

    function doSee(id){
        var form1 = $("form[name='form1']")[0];
        form1.action = "${basePath}nsfw/complain_dealUI.action?complain.compId=" + id + "&isSee=true";
        form1.submit();
    }

    function doAnnualStatistic(){
        var form1 = $("form[name='form1']")[0];
        form1.action = "${basePath}nsfw/complain_annualStatisticChartUI.action";
        form1.submit();
    }

</script>
</body>
</html>